<div id="roster" ng-controller="RosterController as rosterCtrl">  
    <ul id="rosterlist" class="offlineshown active all">
        ...
        <div ng-if="contacts != null && !group.tombstone" ng-repeat="group in contacts" id="group{{group.agroup}}" ng-class="{groupshown: rosterCtrl.groupIsShown(group.agroup)}" >
            ...
            <li ng-repeat="myjid in group.agroupitems" ng-if="!myjid.tombstone" id="{{myjid.ajid}}" class="{{myjid.ajiditems.rosterview.presencetxt}} {{myjid.ajiditems.rosterview.inactive}}" ng-attr-title="{{rosterCtrl.getContactTitle(myjid.ajiditems)}}" ng-class="{condensed: myjid.ajiditems.status != '' && myjid.ajiditems.status != null }" ng-class="rosterCtrl.getContactClient(myjid.ajiditems)" >
                <div class="control" ng-switch on="myjid.ajiditems.rosterview.type">
                ...
                <p class="wrap">
                    <span ng-if="myjid.ajiditems.status != ''">{{myjid.ajiditems.status}}</span>
                </p>
            </li>
        </div>
    </ul>
