...
class Visio extends Base
{
    public function load()
    {
        ...
        $this->registerEvent('presence_muc_muji_handle', 'onMucMujiHandle');
    }
    ...
    public function onMucMujiHandle($packet)
    {
        $this->rpc('MovimJingles.mujiMucPresenceReceived');
    }
    ...
    public function onMujiPresence($packet)
    {
        ...
        && $stanza->muji->content && $stanza->muji->content->attributes()->xmlns == 'urn:xmpp:jingle:1') {
            $this->rpc('MovimJingles.initSession', \baseJid($packet->from), $packet->from);
        }
    }
