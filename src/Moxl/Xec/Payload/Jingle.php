...
class Jingle extends Payload
{
    public function handle(?\SimpleXMLElement $stanza = null, ?\SimpleXMLElement $parent = null)
    {
        ...
        if (
            ...
            && $stanza->muji && (string)$stanza->muji->attributes()->xmlns == 'urn:xmpp:jingle:muji:0'
            && (string)$stanza->muji->attributes()->room == CurrentCalls::getInstance()->mujiRoom
        ) {
            CurrentCalls::getInstance()->startMuji(
                (string)$stanza->muji->attributes()->room,
                $from,
                (string)$stanza->attributes()->sid
            );
        }
