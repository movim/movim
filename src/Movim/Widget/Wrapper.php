...
class Wrapper
{
     ...
     */
    public function setView(string $view): Wrapper
    {
        $this->_view = $view;
        return $this;
    }

    public function setUser(User $user): Wrapper
    {
        $this->user = $user;
        return $this;
    }
    ...
    public function loadWidget(string $name, bool $register = false)
    {
        ...
        if ($register) {
            $widget = new $name($this->user, true);
            $widget->boot();
            ...
            $widget = $this->_view != ''
                ? new $name($this->user, false, $this->_view)
                : new $name($this->user);
    ...
    public function iterate(string $key, ?Packet $packet = null)
    {
        ...
        if (array_key_exists($key, $this->_events)) {
            foreach ($this->_events[$key] as $widgetName) {
                $widget = new $widgetName($this->user, true);
