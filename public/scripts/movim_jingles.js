...
MovimJingleSession.prototype.toggleAudio = function () {
    let rtc = this.pc.getSenders().find(rtc => rtc.track && rtc.track.kind == 'audio');
...
MovimJingleSession.prototype.toggleVideo = function () {
    let rtc = this.pc.getSenders().find(rtc => rtc.track && rtc.track.kind == 'video');
    let mid = this.pc.getTransceivers().filter(t => t.sender.track.id == rtc.track.id)[0].mid;
...
var MovimJingles = {
    sessions: {},
    mucPresenceReceived: false,

    mujiMucPresenceReceived: function() {
        MovimJingles.mucPresenceReceived = true;
    },
    ...
    toggleAudio: function () {
        ...
        for (jid of Object.keys(MovimJingles.sessions)) {
            status = MovimJingles.sessions[jid].toggleAudio();
        }
    ...
    toggleVideo: function () {
        ...
        for (jid of Object.keys(MovimJingles.sessions)) {
            status = MovimJingles.sessions[jid].toggleVideo();
        }
    ...
    onTerminate: function (jid) {
        if (MovimJingles.sessions[jid] == undefined) {
            throw Error('Terminate on a non initiated session ' + jid);
        }
